...
{
	private static RemoteImageManager2 mInstance = null;
    ...
    public RemoteImageObject addImage(ImageView image, String filepath, RelativeLayout loader, TextView failed_to_load, View containerView, int position)
    {
    	// load from cache
    	...
    	{   		
    		...
    		if (bmp.get() != null)
    		{
    			...
    			return new RemoteImageObject(image, filepath, loader, failed_to_load, containerView, position);
    		}    		
    	}    	
		...
		// add to queue
    	RemoteImageObject obj = new RemoteImageObject(image, filepath, loader, failed_to_load, containerView, position);
    	images.add(obj);
    ...
    private class fetchBitmap extends AsyncTask<String, Void, Bitmap>
    {
        ...
        {	
        	...
        	if (!currentImage.aborted) {
					if(currentImage.view.getId() == currentImage.position) {
    ...
    public class RemoteImageObject
    {
    	...
    	public RemoteImageObject(ImageView view, String filepath, RelativeLayout loader, TextView failed_to_load, View containerView, int position)
    	{
    		this.view = view;
